...
class StaticMixModal extends React.Component<Props, State> {
  ...
  render(): JSX.Element | null {
    const { model, vocals, piano, drums, bass, other, errors, softmask, softmask_alpha, isCreating } = this.state;
    ...
    const allChecked = vocals && piano && drums && bass && other;
    const noneChecked = !(vocals || piano || drums || bass || other);
    ...
    return (
      <Modal show={show} onHide={!isCreating ? this.onHide : undefined} onExited={this.onExited}>
        <Modal.Header closeButton>
          <Modal.Title>Criar static mix</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <StaticMixModalForm
            song={song}
          ...
          />
          {allChecked && <Alert variant="warning">Você deve deixar pelo menos uma parte desmarcada.</Alert>}
          {noneChecked && <Alert variant="warning">Você deve selecionar pelo menos um instrumento.</Alert>}
          {slowCpuModel && <Alert variant="warning">Este modelo tem duração de separação muito longo.</Alert>}
          {invalidAlpha && <Alert variant="danger">O alfa de softmask deve ser maior que 0.</Alert>}
          {errors.length > 0 && (
