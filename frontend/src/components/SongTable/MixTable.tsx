...
import { AccompBadge, AllBadge, BassBadge, DrumsBadge, VocalsBadge, PianoBadge } from '../Badges';
...
const statusColFormatter: ColumnFormatter<MixItem> = (cell, row, rowIndex) => {
  ...
  if (row.mix.error) {
    ...
    finishedDateTimeText += `Erro: ${row.mix.error}`;
  }
    ...
    if (data.length > 0) {
      ...
      return (
        <div className="m-4 text-center">
          <p>Sem mixagens. Clique em &ldquo;Dynamic Mix&rdquo; ou &ldquo;Static Mix&rdquo; para criar uma mix.</p>
        </div>
      );
    }
  }
