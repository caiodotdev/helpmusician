...
const CustomPreview = ({
          ...
          <div className="col">
            <div className="d-flex">
              {status === 'error_file_size' && (
                <span className="dzu-sizeError">{size < minSizeBytes ? 'Arquivo muito pequeno!' : 'Arquivo muito grande!'}</span>
              )}
              {status === 'error_validation' && <span>{String(validationError)}</span>}
              {canRemove && (
                <span className="dzu-previewButton align-self-center" style={iconByFn.remove} onClick={remove} />
              )}
            </div>
          </div>
        </div>
  ...
  if (status === 'error_upload_params' || status === 'exception_upload' || status === 'error_upload') {
    title = `${title} (falha no upload)`;
    variant = 'danger';
  }

  if (status === 'aborted') {
    title = `${title} (cancelado)`;
    variant = 'secondary';
  }
